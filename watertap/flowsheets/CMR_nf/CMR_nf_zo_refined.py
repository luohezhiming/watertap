...
def CMR_nf_case(case=Case.case1, simplified_routine=False):
    ...
    print("Model DOF = " + str(degrees_of_freedom(m)))
    set_scaling(m)
    print("after scaling Model DOF = " + str(degrees_of_freedom(m)))
    initialize_system(m)
    # stripper = VariableBoundStripper()
    # stripper.apply_to(m)
    print("before solve Model DOF = " + str(degrees_of_freedom(m)))
    m, results = solve(m)
    print("after solve Model DOF = " + str(degrees_of_freedom(m)))
    # m.fs.NF.area.unfix()
